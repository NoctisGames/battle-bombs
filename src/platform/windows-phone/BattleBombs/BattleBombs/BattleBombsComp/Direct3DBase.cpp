...
void Direct3DBase::touchDown(float x, float y)
{
	addTouchEventForType(DOWN, x, y);
}

void Direct3DBase::touchDragged(float x, float y)
{
	if (m_touchEventsBuffer.size() < 3)
	{
		addTouchEventForType(DRAGGED, x, y);
	}
}

void Direct3DBase::touchUp(float x, float y)
{
	addTouchEventForType(UP, x, y);
}
...
void Direct3DBase::Update(Direct3DGameScreen &gameScreen, float timeDelta)
{
	...
	gameScreen.update(timeDelta, m_touchEvents);
}
...
void Direct3DBase::addTouchEventForType(Touch_Type touchType, float x, float y)
{
...
TouchEvent Direct3DBase::newTouchEvent()
{
